...
const createInvoice = async (req, res) => {
  try {
    const { booking_id, room_charges, service_charges, payment_method, payment_status } = req.body;
    ...
    const invoice = await Invoice.create({
      booking_id,
      room_charges,
      service_charges: service_charges && service_charges.length > 0 ? service_charges : null,
